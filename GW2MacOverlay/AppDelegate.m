...
-(void)createMenu{
    ...
    NSMenuItem *dingDongModeItem = [[NSMenuItem alloc] initWithTitle:@"Ding Dong" action:@selector(setMode:) keyEquivalent:@""];
    
    [normalModeItem setTag:0];
    [dingDongModeItem setTag:1];
    
    [modeMenu addItem:normalModeItem];
    [modeMenu addItem:dingDongModeItem];
...
-(IBAction)setMode:(id)sender{
    ...
    // Update UI
    self.masterViewController._isDingDong = [self._currentMode tag];
    [self.masterViewController._statusTable reloadData];
    
    NSLog(@"%hhd", self.masterViewController._isDingDong);
}
